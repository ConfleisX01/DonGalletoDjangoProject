...
class DashboardVentasView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        inventarios = InventarioMaterial.objects.annotate(
            costo_total=Sum(
                F('insumo__lotes__costo_unitario') * F('cantidad')
            )
        )
        ...
        # Recetas más vendidas en toda la historia
        top_recetas = (
            VentaDetalle.objects
            .values('receta__nombre')
            .annotate(
                total_vendidas=Sum('cantidad'),
                total_vendido=Sum('total')
            )
            .order_by('-total_vendidas')
        )

        # Cantidad total histórica vendida de todas las recetas
        cantidad_total_vendida = (
            VentaDetalle.objects
            .aggregate(total_vendida=Sum('cantidad'))['total_vendida'] or 0
        )
        ...
        context.update({
            ...
            'fecha_fin': fecha_fin,
            'top_recetas': top_recetas,  # Añadimos las recetas más vendidas históricamente
            'cantidad_total_vendida': cantidad_total_vendida  # Añadimos la cantidad total histórica vendida
        })
